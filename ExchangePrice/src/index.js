...
import TradeFactory from "../build/contracts/catalanExchange.json";
...
var ofTrade;
const amount = new BigNumber(1e+17).toFixed();
...
const refresh = () => {
    ...
    if(btnGroupDrop1.innerHTML == "ETH"){
      ...
      if(btnGroupDrop2.innerHTML == "DAI"){
        ...
        CatalanExchange.methods.priceDaiToEth2(tokenIn).call({from: accounts[0]})
        .then(x => {
      ...
      if(btnGroupDrop2.innerHTML == "KNC"){
        ...
        CatalanExchange.methods.priceKncToEth2(tokenIn).call({from: accounts[0]})
        .then(x => {
          let r = new BigNumber(x[1]).dividedBy(unity);
          input2.value = r.toFormat(6);
      });
      ...
      if(btnGroupDrop2.innerHTML == "MANA"){
        ...
        CatalanExchange.methods.priceManaToEth2(tokenIn).call({from: accounts[0]})
        .then(x => {
          let r = new BigNumber(x[1]).dividedBy(unity);
          input2.value = r.toFormat(6);
      });
    ...
    if(btnGroupDrop1.innerHTML == "DAI"){
      ...
      if(btnGroupDrop2.innerHTML == "KNC"){
        bgKncOut.hidden = false;
        CatalanExchange.methods.priceDaiToKnc2(tokenIn).call({from: accounts[0]})
        .then(x => {
          let r = new BigNumber(x[0]).dividedBy(unity);
          input2.value = r.toFormat(6);
      });
      ...
      if(btnGroupDrop2.innerHTML == "MANA"){
        bgManaOut.hidden = false;
        CatalanExchange.methods.priceManaToDai2(tokenIn).call({from: accounts[0]})
        .then(x => {
          let r = new BigNumber(x[1]).dividedBy(unity);
          input2.value = r.toFormat(6);
      });
    ...
    if(btnGroupDrop1.innerHTML == "KNC"){
      ...
      if(btnGroupDrop2.innerHTML == "MANA"){
        bgManaOut.hidden = false;
        CatalanExchange.methods.priceManaToKnc2(tokenIn).call({from: accounts[0]})
        .then(x => {
          let r = new BigNumber(x[1]).dividedBy(unity);
          input2.value = r.toFormat(6);
      });
...
async function sendSign(transaction, value = 0){
  const options = {
        to      : TradeFactory.networks[deploymentKey].address,
...
testButton.addEventListener('click', () =>{
  ...
  CatalanExchange.methods.buyDaiToEth2(amount).send({from: accounts[0]})
  .then(x => {
    console.log(x);
  })
  /*
...
approveButton.addEventListener('click', function(){
  ...
  let tokenIn = new BigNumber(input1.value).multipliedBy(unity);
  if(btnGroupDrop1.innerHTML == "DAI"){
    DaiContract.methods.approve("0x7AFcac971A4EA02372e8Bb22D41bcB7f0047C7b0", tokenIn).send({from: accounts[0]})
    .then(x => {
      clearInterval(loop);
      doButton.hidden = false;
      approveButton.hidden = true;
  ...
  if(btnGroupDrop1.innerHTML == "KNC"){
    KncContract.methods.approve("0x7AFcac971A4EA02372e8Bb22D41bcB7f0047C7b0", tokenIn).send({from: accounts[0]})
    .then(x => {
      doButton.hidden = false;
      approveButton.hidden = true;
    });
  ...
  if(btnGroupDrop1.innerHTML == "MANA"){
    ManaContract.methods.approve("0x7AFcac971A4EA02372e8Bb22D41bcB7f0047C7b0", tokenIn).send({from: accounts[0]})
    .then(x => {
      doButton.hidden = false;
      approveButton.hidden = true;
    });
...
doButton.addEventListener('click', function(){
  ...
  if (btnGroupDrop2.innerHTML == btnGroupDrop1.innerHTML){
    ...
    if(btnGroupDrop1.innerHTML == "ETH"){
      if(btnGroupDrop2.innerHTML == "DAI"){
        CatalanExchange.methods.buyEthToDai2(tokenIn).send({from: accounts[0], value: amount})
        .then(x => {
          transfeDone.hidden = false;
        });
      }
      if(btnGroupDrop2.innerHTML == "KNC"){
        CatalanExchange.methods.buyEthToKnc2(tokenIn).send({from: accounts[0], value: amount})
        .then(x => {
          transfeDone.hidden = false;
        });
      }
    ...
    if(btnGroupDrop1.innerHTML == "DAI"){
      if(btnGroupDrop2.innerHTML == "ETH"){
        CatalanExchange.methods.transferRapid(tokenIn).send({from: accounts[0]})
        .then(p => {
          approveButton.hidden = false;
          doButton.hidden = true;
          transfeDone.hidden = true;
          var temps = setTimeOut(function() {loop = setInterval(refresh, 500);}, 5000);
...
/*
    DaiContract.methods.approve("0x29679Aa3B9D83F6210faF7f79D9aa7E0589cA514", amount).send({from: accounts[0]})
    .then(x => {
      DaiContract.methods.transferFrom(accounts[0], "0x29679Aa3B9D83F6210faF7f79D9aa7E0589cA514", amount).send({from: accounts[0]})
      .then(p => {
        console.log(p);
